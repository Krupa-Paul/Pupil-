function [state_of_light_right state_of_light_left area_pupil_right area_pupil_left] = area_of_pupil_allvideos(video_right,video_left)
    ...
    for k = 1:1:n
        ...
        %imshow(I_fill)
          ...
          %[centersBright, radiiBright] = imfindcircles(I_fill, [Rmin Rmax], ...
                                      %  'ObjectPolarity','bright');
  
          % Find all the dark circles in the image
          %[centersDark, radiiDark] = imfindcircles(I_fill, [Rmin Rmax], ...
                                       % 'ObjectPolarity','dark');
        ...
        if size(centers) == 0
            ...
            % Uncomment the below 7 lines to see the contour overlayed on
            ...
            %f=getframe(gca)
            %[X, map] = frame2im(f);
            %fname_frame = strcat('frame',num2str(k),'.jpg');
            %imwrite(X,fullfile('..\fname1',fname_frame));
        end
    ...
    for k = 1:1:n
        ...
        if size(centers) == 0
            ...
            %[X, map] = frame2im(f);
            %fname_frame = strcat('frame',num2str(k),'.jpg');
            %imwrite(X,fullfile('..\fname2',fname_frame));
        end
